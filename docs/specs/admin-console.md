...
- _(P4)_ `membership`, `waiting_room`, `meeting.status` — admin cũng đọc/ghi.
...
- ✅ **A3 (chín):** **System settings** (domain nội bộ, waiting-room/recording mặc định, retention) + **Analytics** + **Security overview**. _(Compliance/GDPR + theo dõi đăng nhập sai = cần log-drains → để sau; Announcements = sau.)_
