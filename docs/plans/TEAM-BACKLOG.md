...
| 25 | PHONE (mobile) mic OFF + can't enable, audio-call join hangs/frozen — iPad OK, camera/screen-share OK. Recording feature does NOT touch these audio files → likely responsive/CSS (iPhone) OR getUserMedia-blocks-join on phone. Investigating. | ☐ investigating | FE+Debugger | `audio/useJoinCall.ts`, `audio/DailyAudio.ts`, `MeetingCallControls.tsx`, `MeetingShell.scss` |
