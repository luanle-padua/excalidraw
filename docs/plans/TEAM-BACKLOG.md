...
| 25b | iPhone audio-call join "rất lâu rồi cũng vào" (slow, not permanent hang) — STILL OPEN. Suspect `AudioRoomController` STT auto-`ensureMic` calling getUserMedia OFF a user gesture on iOS (hangs/blocks until timeout). Verify AFTER #25 mic fix; if persists, gate STT auto-acquire behind a gesture / add a join timeout. | ☐ verify after #25 | FE+Debugger | `audio/AudioRoomController.tsx`, `audio/DailyAudio.ts` |
