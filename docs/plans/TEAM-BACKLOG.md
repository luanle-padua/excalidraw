...
| 17 | Consent gate leaking onto dashboard — must render ONLY on meeting join (owner saw it pop on dashboard) | ☐ | FE+Debugger | `MeetingConsentGate.tsx`, `MeetingShell.tsx` |
| 18 | Consent copy rewrite — smart/light/persuasive so users click OK instantly (not a dry explicit notice) | ☐ | UX+FE | `MeetingConsentGate.tsx`, i18n `en/ko/vi` |
| 19 | Replay entry: clicking "Tua lại" shows the player control bar at the bottom directly — drop the intermediate menu + extra click | ☐ | FE | `CanvasReplay*`, `MeetingLogModal` |
| 20 | JOIN flow optimization (-2~4s): merge 3 `getMeeting` calls into 1 cached atom + run `getMeetingChecked` parallel to socket | ☐ | FE | `Collab.tsx` |
