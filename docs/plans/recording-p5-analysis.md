...
6. **🆕 ĐỊNH DẠNG + TỐI ƯU DUNG LƯỢNG (recording CHỈ để review lại / làm tài liệu dự án — KHÔNG cần chất lượng phát sóng):** đầu ra Daily = **MP4 (H.264 + AAC)**. Vì chỉ xem lại/tư liệu → **ghi độ phân giải + bitrate THẤP** (vd 720p hoặc 480p, bitrate thấp, fps vừa phải, layout gọn) để file nhỏ nhất có thể. Lưu **R2 Infrequent-Access tier** (rẻ cho ít truy cập) + **lifecycle auto-xoá sau N ngày** (retention D3). **Canvas** dùng phương án (c) **replay từ event-log = ~0 storage** (không tạo file video canvas). Mục tiêu rõ ràng của anh Luân: **dung lượng tối thiểu**.
