# Migration realtime: socket.io → Cloudflare Durable Objects (DO) (DONE 06-17 — 100% DO live)
...
>
> **CHỐT 06-17 (REVISED) — ĐÃ HOÀN TẤT: realtime TOÀN BỘ 100% DO. KHÔNG Fly, KHÔNG socket.io. Thư mục `room/` đã GỠ HẲN.** DO chạy ngay trên Worker `mcm-storage` (không host riêng, không tốn host). Cờ `realtime_backend` giờ chỉ còn cho admin rollout view — client luôn resolve "do" (xem `resolveRealtimeBackend`); rollback về socketio không còn khả thi. _(Mọi nhắc tới "Fly.io" / "giữ socket.io làm rollback lâu dài" ở các mục dưới = framing CŨ đã huỷ — bỏ qua.)_
>
