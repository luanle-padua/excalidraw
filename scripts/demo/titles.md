# Demo clip titles — copy-paste ready

> Mỗi phân đoạn em đề xuất **2–3 option** (VN/EN). Anh pick 1, hoặc trộn (VN title + EN sub-line tagline). Kiểu Apple keynote: 2–4 chữ, ngắt dòng có chủ ý, dùng dấu chấm.
...
## 1. Vignette 1 — Cùng làm việc trên một bản vẽ
...
> Đây là beat mạnh nhất Vignette 1, em vote VN "Mỗi người một chi tiết" hoặc EN "Same plan. Different focus." cho impact.

### Beat 1c — PDF viewer + note + mark
...
| Punchy | `PDF — không cần rời canvas.` | `PDFs, without leaving the canvas.` |
...
| Punchy | `Một dòng, đúng chỗ.` | `Right where it matters.` |
...
| Punchy | `Một dấu — ba ngôn ngữ.` | `One mark. Every language.` |
...
## 2. Vignette 2 — Communication không rào cản
...
## 3. Vignette 3 — AI Summary (chưa quay, prep title)
...
> Recommend A nếu external (gửi cho khách hàng / công ty khác), C nếu nội bộ MAP.

---

## Khuyên cho production

- **Font card**: dùng `Caveat 700` cho VN/EN — match font canvas của app, giữ identity nhất quán. Hoặc `Inter 600` cho clean sans-serif.
- **Vị trí**: title ở giữa-trên (1/3 trên), sub-line nhỏ hơn ở dưới title 12pt.
- **Animation**: fade-in 200ms + slide-up 8px, fade-out 150ms. Hold ~2s rồi cut sang canvas.
- **Màu**: trắng trên overlay 60% đen, hoặc đen trên trắng nếu canvas đang sáng. Tránh accent màu để không lấn nội dung canvas.
- **Ngôn ngữ trộn**: nếu video target VN, em vote VN title + EN sub-line cho "international feel" (vd: `Cùng vẽ. Cùng góp ý.` + sub `Real-time, side by side.`)

---

## Cheat sheet — quick picks (em vote)

```
0.  Hook       → "Một cuộc họp. Một bản vẽ."
1.  V1 title   → "Cùng vẽ. Cùng góp ý."
1a             → "Khoanh chỗ. Kéo hình."
1b             → "Mỗi người một chi tiết."
1c-i  PDF mở  → "Bản spec mở thẳng trên canvas."
1c-ii note    → "Ghi chú trên PDF. Như trên giấy."
1c-iii mark   → "Khoanh chỗ — dịch luôn."
1d             → "Đóng dấu — dính luôn."
2.  V2 title   → "Ba ngôn ngữ. Một cuộc họp."
2a             → "Gõ một lần. Đọc theo ngôn ngữ của bạn."
2b             → "Nói gì, hiện đó. Trên cả ba ngôn ngữ."
3.  V3 title   → "Cuộc họp tự tóm tắt."
3 sub          → "Quyết định. Việc cần làm. Trong vài giây."
4.  End        → "MAP CanvasMeet."  +  sub "Họp cùng nhau. Quyết cùng nhau."
```
